@tool
@icon("res://resources/editor/audio_stacking.svg")
...
@export var max_audio_playback: int = 2:
	set = set_max_audio_playback
@export var sound_resource: Resource: #AudioResource
	set = set_sound_resource
...
@export var override_sound: bool = false 
@export var default_bus: String = "Master"
@export_range(-100.0, 0.0) var default_volume: float = 0.0
@export_range(0.0, 4.0) var default_pitch_scale: float = 1.0
# todo mix target

var playing_in_editor = false: 
	set = set_playing_in_editor # only use in editor, has no effect ingame
var _audio_streams = []:
	set = private_set
...
static func _override_sound(audio_streams, audio_resource, volume_dB = 0.0, pitch_scale = 1.0, bus = "Master"):
...
func _add_audio_stream():
	...
	audio_stream_player.connect("finished", _on_finished)
...
func set_playing_in_editor(new_bool):
	if Engine.is_editor_hint():
		...
		if playing_in_editor:
			...
			node.connect("finished", Callable(self,"_on_sound_finished_editor").bind([]), 4) # CONNECT_ONESHOT
